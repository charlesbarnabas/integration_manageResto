document.addEventListener('DOMContentLoaded', function() {
  ...
  function fetchOrderTransactions(query = '') {
    fetch('/api/transaction/transactions')
      ...
      .then(data => {
        txTbody.innerHTML = '';
        data.filter(tx => tx.method.toLowerCase().includes(query.toLowerCase()) || String(tx.order_id).includes(query))
          .forEach(tx => {
  ...
  function fetchProcurementTransactions(query = '') {
    fetch('/api/transaction/procurement')
      ...
      .then(data => {
        ...
        data.filter(tx => tx.ingredient_name.toLowerCase().includes(query.toLowerCase()) || String(tx.procurement_order_id).includes(query))
          .forEach(tx => {
            const tr = document.createElement('tr');
            tr.innerHTML = `
              <td>${tx.id}</td>
